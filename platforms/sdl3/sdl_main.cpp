//
// Copyright (c) 2025 Liam Reese
//

#include <cstdlib>
#include <cstdio>

#include <SDL3/SDL.h>

#include <rt_everywhere.hpp>

#include <pipeline/rte_framebuffer.hpp>

#include <glm/glm.hpp>

#include <imgui.h>
#include <backends/imgui_impl_sdl3.h>
#include <backends/imgui_impl_sdlrenderer3.h>

//#define LOW_RES_FB

#ifdef LOW_RES_FB

#define PREVIEW_SIZE_X (640 >> 2)
#define PREVIEW_SIZE_Y (480 >> 2)

#else

#define PREVIEW_SIZE_X (640)
#define PREVIEW_SIZE_Y (480)

#endif

bool shouldRender = 0;

glm::vec3 cameraEuler;
rteCamera camera;

class rteSDLFramebuffer : public rteFramebuffer {

protected:
    SDL_Texture* sdlTexture = nullptr;
    int width, height;

    uint8_t* pPixels = nullptr;

    // Only supports ARGB textures!

public:
    ~rteSDLFramebuffer() {
        SDL_DestroyTexture(this->sdlTexture);
    }

    rteSDLFramebuffer(SDL_Texture* sdlTexture) {
        this->sdlTexture = sdlTexture;

        float rawX, rawY;
        SDL_GetTextureSize(sdlTexture, &rawX, &rawY);

        this->width = (int)rawX;
        this->height = (int)rawY;
    }

    bool IsThreadSafe() const override {
        return true;
    }

    int GetWidth() const override {
        return width;
    }

    int GetHeight() const override {
        return height;
    }


    void WritePixel(int x, int y, glm::vec4 color) override {

        if (pPixels == nullptr) {
            return; // Pixels aren't locked
        }

        if (x >= width || y >= height) {
            return; // OOB
        }

        int index = (y * width) + x;
        index *= 4;

        pPixels[index + 2] = (uint8_t)(color.r * 255.0F);
        pPixels[index + 1] = (uint8_t)(color.g * 255.0F);
        pPixels[index + 0] = (uint8_t)(color.b * 255.0F);
        pPixels[index + 3] = (uint8_t)(color.a * 255.0F);

    }


    void Lock() override {
        if (sdlTexture == nullptr) {
            return;
        }

        SDL_Rect rect;

        rect.x = 0;
        rect.y = 0;

        rect.w = width;
        rect.h = height;

        int pitch;
        SDL_LockTexture(sdlTexture, &rect, reinterpret_cast<void **>(&pPixels), &pitch);
    }

    void Unlock() override {
        if (sdlTexture == nullptr) {
            return;
        }

        SDL_UnlockTexture(sdlTexture);
    }

    void SDLRender(SDL_Renderer* renderer) {
        SDL_RenderTexture(renderer, sdlTexture, nullptr, nullptr);
    }

    SDL_Texture* GetSDLTexture() const {
        return sdlTexture;
    }
};

class rteViewport {
public:
    bool isOpen = true;
    float viewPercentage = 1.0F;

    rteSDLFramebuffer* framebuffer = nullptr;

    void DrawViewport(SDL_Renderer* sdlRenderer) {

        // TODO: Use a random id to ensure viewport clashing doesn't occur
        if (!ImGui::Begin("Viewport", &isOpen, ImGuiWindowFlags_MenuBar)) {
            ImGui::End();
            return;
        }

        ImGui::BeginMenuBar();

        if (ImGui::BeginMenu("Viewport")) {
            ImGui::BeginPopup("VIEWPORT_CAMERA_PANE");

            ImGui::DragFloat("Render Scale", &viewPercentage, 0.01F, 0.1F, 2.0F);

            ImGui::EndMenu();
        }

        ImGui::EndMenuBar();

        ImVec2 imSize = ImGui::GetContentRegionAvail();

        int scaledWidth = imSize.x * viewPercentage;
        int scaledHeight = imSize.y * viewPercentage;

        if (imSize.x == 0 || imSize.y == 0) {
            ImGui::SetWindowSize({64, 64});
            ImGui::End();
            return;
        }

        bool dirtyFB = framebuffer == nullptr;

        if (!dirtyFB && (scaledWidth != framebuffer->GetWidth() || scaledHeight != framebuffer->GetHeight())) {
            dirtyFB = true;
        }

        if (dirtyFB) {
            SDL_Texture* texture = SDL_CreateTexture(
                    sdlRenderer,
                    SDL_PIXELFORMAT_ARGB8888,
                    SDL_TEXTUREACCESS_STREAMING,
                    scaledWidth,
                    scaledHeight
            );

            SDL_SetTextureScaleMode(texture, SDL_SCALEMODE_NEAREST);

            delete framebuffer; // TODO: When async code comes, ensure this isn't in use
            framebuffer = new rteSDLFramebuffer(texture);
        }

        ImGui::Image(reinterpret_cast<ImTextureID>(framebuffer->GetSDLTexture()), imSize);

        ImGui::End();

    }

};

int main(int argc, char** argv) {
    SDL_Init(0);

    SDL_Window* window = SDL_CreateWindow(
        "RT Everywhere (SDL)",
        1024,
        768,
        SDL_WINDOW_RESIZABLE
    );

    if (window == NULL) {
        printf("FATAL: SDL Window was NULL!\n");
        return 1;
    }

    SDL_DisplayID primDisplay = SDL_GetPrimaryDisplay();

    SDL_Rect displayRect {};
    if (!SDL_GetDisplayBounds(primDisplay, &displayRect)) {
        SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR , "Failed to get display!", SDL_GetError(), nullptr);
        return 1;
    }

    SDL_SetWindowSize(window, displayRect.w >> 1, displayRect.h >> 1);

    const char* sdl_driver_name = "Software";
    //const char* sdl_driver_name = nullptr;

    SDL_Renderer* renderer = SDL_CreateRenderer(window, sdl_driver_name);

    if (renderer == NULL) {
        printf("FATAL: SDL Renderer was NULL!\n");
        return 1;
    }

    SDL_Texture* previewTexture = SDL_CreateTexture(
            renderer,
            SDL_PIXELFORMAT_ARGB8888,
            SDL_TEXTUREACCESS_STREAMING,
            PREVIEW_SIZE_X,
            PREVIEW_SIZE_Y
    );

    rteSDLFramebuffer previewFB(previewTexture);

    rteState state;

    // TODO: This may become more complex later!
    state.Setup();
    camera.transform.SetPosition(glm::vec3(0, 1, 0));

    ImGuiContext* imgui_context = ImGui::CreateContext();
    ImGui::SetCurrentContext(imgui_context);

    ImGuiIO& io = ImGui::GetIO();
    io.ConfigFlags |= ImGuiConfigFlags_DockingEnable;

    ImGui_ImplSDL3_InitForSDLRenderer(window, renderer);
    ImGui_ImplSDLRenderer3_Init(renderer);

    SDL_Event event;
    int run = 1;

    int lock_cursor = 0;
    int recreate_texture = 1;

    uint32_t last;
    uint32_t now = (uint32_t)SDL_GetTicks();

    int manual_width = 320;
    int manual_height = 240;
    int manual_size = 0;

    int w_down = 0;
    int s_down = 0;
    int a_down = 0;
    int d_down = 0;
    int q_down = 0;
    int e_down = 0;

    rteViewport viewport;

    while (run) {
        int draw_preview = 1;

        //https://gamedev.stackexchange.com/questions/110825/how-to-calculate-delta-time-with-sdl
        last = now;
        now = (uint32_t)SDL_GetPerformanceCounter();
        float delta_time = (float) (now - last) / (float) SDL_GetPerformanceFrequency();

        while (SDL_PollEvent(&event)) {
            if (event.type == SDL_EVENT_QUIT) {
                run = 0;
                break;
            }

            if (event.type == SDL_EVENT_WINDOW_RESIZED) {
                recreate_texture = 1;
            }

            if (event.type == SDL_EVENT_KEY_DOWN) {
                if (event.key.scancode == SDL_SCANCODE_W) {
                    w_down = 1;
                }

                if (event.key.scancode == SDL_SCANCODE_S) {
                    s_down = 1;
                }

                if (event.key.scancode == SDL_SCANCODE_A) {
                    a_down = 1;
                }

                if (event.key.scancode == SDL_SCANCODE_D) {
                    d_down = 1;
                }

                if (event.key.scancode == SDL_SCANCODE_Q) {
                    q_down = 1;
                }

                if (event.key.scancode == SDL_SCANCODE_E) {
                    e_down = 1;
                }
            }

            if (event.type == SDL_EVENT_KEY_UP) {
                if (event.key.scancode == SDL_SCANCODE_W) {
                    w_down = 0;
                }

                if (event.key.scancode == SDL_SCANCODE_S) {
                    s_down = 0;
                }

                if (event.key.scancode == SDL_SCANCODE_A) {
                    a_down = 0;
                }

                if (event.key.scancode == SDL_SCANCODE_D) {
                    d_down = 0;
                }

                if (event.key.scancode == SDL_SCANCODE_Q) {
                    q_down = 0;
                }

                if (event.key.scancode == SDL_SCANCODE_E) {
                    e_down = 0;
                }
            }

            if (event.type == SDL_EVENT_MOUSE_MOTION) {
                if (lock_cursor) {
                    static glm::vec3 euler;// = glm::degrees(camera.transform.GetEulerRotation());

                    euler.x -= event.motion.yrel / 10.0F;
                    euler.y -= event.motion.xrel / 10.0F;
                    euler.z = 0;

                    camera.transform.SetEulerRotation(glm::radians(euler));
                }
            }

            if (event.type == SDL_EVENT_MOUSE_BUTTON_DOWN) {
                if (event.button.button == SDL_BUTTON_RIGHT) {
                    lock_cursor = !lock_cursor;
                    SDL_SetWindowRelativeMouseMode(window, lock_cursor);

                    if (lock_cursor == 0) {
                        shouldRender = 1;
                    }
                }
            }

            ImGui_ImplSDL3_ProcessEvent(&event);
        }

        if (lock_cursor) {
            draw_preview = 1;
        }

        if ((w_down || s_down || a_down || d_down || q_down || e_down) && lock_cursor) {
            glm::vec3 movement = glm::vec3(0.0F);

            if (w_down) {
                movement.z = -1;
            }

            if (s_down) {
                movement.z = 1;
            }


            if (a_down) {
                movement.x = -1;
            }

            if (d_down) {
                movement.x = 1;
            }


            if (q_down) {
                movement.y = -1;
            }

            if (e_down) {
                movement.y = 1;
            }

            SDL_Keymod mod = SDL_GetModState();

            float gear = 1.0F;

            if (mod & (SDL_KMOD_LSHIFT | SDL_KMOD_RSHIFT))
                gear = 4.0F;

            glm::mat4x4 matrix = camera.transform.GetMatrix();
            //matrix = glm::inverse(matrix);

            movement = glm::normalize(movement);
            //printf("%f, %f, %f\n", movement.x, movement.y, movement.z);
            movement = camera.transform.GetRotation() * movement;
            movement *= gear * delta_time;

            camera.transform.SetPosition(camera.transform.GetPosition() + movement);

            // Transform movement vector to be local
        }

        static bool renderTabOpen = true;

        ImGui_ImplSDL3_NewFrame();
        ImGui_ImplSDLRenderer3_NewFrame();
        ImGui::NewFrame();

        ImGui::Begin("Render", &renderTabOpen);

        ImGui::Text("LAST HIGHEST SHADE TIME %fms (%llius)", state.highestShadeTime / 1000.0F, state.highestShadeTime);

        if (ImGui::Button("FORCE RENDER")) {
            shouldRender = 1;
        }

        ImGui::Checkbox("Bounce Heatmap?", &state.visualizeBounceHeat);
        ImGui::Checkbox("Shade Heatmap?", &state.visualizeShadeTime);

        constexpr int TOTAL_SHADE_TIMES = 60;
        static float lastShadeTimes[TOTAL_SHADE_TIMES];

        if (lock_cursor || shouldRender) {

            rteRenderTarget rt;

            rt.pFramebuffer = viewport.framebuffer;
            rt.camera = camera;

            state.Render(rt);
            shouldRender = 0;

            for (int s = 0; s < TOTAL_SHADE_TIMES - 1; s++)
                lastShadeTimes[s] = lastShadeTimes[s + 1];

            lastShadeTimes[TOTAL_SHADE_TIMES - 1] = state.highestShadeTime;

        }

        ImGui::PlotLines("SHADE TIMES", reinterpret_cast<const float *>(lastShadeTimes), TOTAL_SHADE_TIMES, 0);

        ImGui::End();

        // TODO: Multiple viewports
        viewport.DrawViewport(renderer);

        SDL_RenderClear(renderer);
        //previewFB.SDLRender(renderer);

        ImGui::Render();
        ImGui_ImplSDLRenderer3_RenderDrawData(ImGui::GetDrawData(), renderer);

        SDL_RenderPresent(renderer);
    }

    return 0;
}